...
<template>
  <div class="brochure">
    ...
    <el-card class="brochure-card" shadow="never" v-loading="historyLoading">
      ...
      >
        <el-table-column label="파일명" min-width="240">
          <template #default="{ row }">
            <div class="history-name">
              <span class="history-name-text">{{ row.file_name }}</span>
              <el-button
                v-if="row.id === currentHistoryId && currentUrl"
                tag="a"
                :href="currentUrl"
                target="_blank"
                size="small"
